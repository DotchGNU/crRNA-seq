#!/usr/bin/env python
# -*- coding: utf-8 -*-
# python count_RNA.py -i input1.txt input2.txt ... -o output.txt
...
def read_parsed_bowtie(file, qscore=0):
    """Reads parsed Bowtie results and counts reference RNA occurrences."""
    bowtie = {}
...
# Argument Parsing
parser = argparse.ArgumentParser(usage='python count_RNA.py [ -i input1.txt input2.txt ... / -d input_dir ] -o output.txt')
...
# Process reference RNA names and mutations
final_df['RNA name#pos:mut'] = final_df.index
final_df[['RNA name', 'pos:mut']] = final_df.pop('RNA name#pos:mut').str.split("#", expand=True)
...
# Sort by RNA name, mismatch number, and mutation positions
final_df = final_df.sort_values(['RNA name', 'mm_numb', 'pos1', 'mut1', 'pos2', 'mut2'])
# Reorder columns
...
tmp_sum = final_df.groupby(['RNA name']).sum()
tmp_sum.columns = [x + ' (PM+1MM+2MM)' for x in tmp_sum.columns]
final_df = pd.merge(final_df, tmp_sum, left_on='RNA name', right_index=True)
