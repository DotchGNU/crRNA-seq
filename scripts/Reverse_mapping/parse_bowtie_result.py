...
""" * Alignment criteria
	1) PM > 1MM > 2MM
 	2) Counts "concatemer" as 1. (where, "concatemer" = >=2 identical RNA sequence (including mismatch) present in a read
    - In case of concatemer, only the first-encountered RNA is considered. (for Q-score)
"""
...
def select_uniquely_mapped_read(bowtie_dict):
	...
	for fq_seq, score_dict in bowtie_dict.iteritems():	# score_dict = { 0: [ ['0','23','mir-1','','11'], ... ] }
		...
		# Concatamer (>1 "identical" RNAs mapped to a read)
		...
		# Unambiguously mapped reads (>1 "different" RNAs mapped to a read)
		else:
			multi_mapped[fq_seq] = score_dict[mm_count]
                        multi_mm_count_list.append(mm_count)

                # if multiple PM reads is exist : warning
                if 0 in multi_mm_count_list:
                    print ("Warning: Some perfect match reads is discarded")
...
q_ref = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~"
...
def correct_bowtie_mut_Qscore(mut, start, qscore):
        if mut == '':
                ...
                for mm in mut.split(','):
                        tmp = mm.split(':')
                        mm_pos = int(tmp[0])			# position according to RNA
...
heading = ['# RNA_name', 'pos:mut:Q-score', 'Q-score', '5p_sequence', '5p_Q-score', '3p_sequence', '3p_Q-score']
