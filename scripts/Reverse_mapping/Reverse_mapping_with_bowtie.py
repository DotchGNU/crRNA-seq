...
# ./Reverse_mapping_with_bowtie.py -i Input_folder/ -r reference_RNA_sequence.fa -o Output_folder/

# bowtie parameter: --norc -l 7 -n 2 -a -f
# All scripts must be present in same directory!
# Scripts used: FastQcollapse.py, parse_bowtie_result.py, count_RNA.py, extract_bowtie_unmapped_reads.py
...
def get_RNA_count(mapped_list, Qscore, dir):	# count only mismatch with its Q-score above threshold
	count_file = dir + 'RNA_count.Q' + str(Qscore) + '.txt'
	run = subprocess.check_output( 'python {0}/count_RNA.py -i {1} -q {2} -o {3}'.format(script_loc, ' '.join(mapped_list), str(Qscore), count_file), shell=True )
...
parser=argparse.ArgumentParser(usage='./Reverse_mapping_with_bowtie.py [options] -r reference_RNA_sequence.fa', formatter_class=argparse.RawTextHelpFormatter)
...
### Reverse_mapping with bowtie & parse output ###
...
mapped_count = get_RNA_count(bowtie_file_list, q_threshold, out_dir)
